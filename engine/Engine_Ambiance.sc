...
// driven by `stability`. `speed` sets the base playback rate.
//
// see ambiance-stability-control.md and ambiance-final-addendum.md.

Engine_Ambiance : CroneEngine {
	var <synth;
	var <buffers;
	var <folder;
	var <server;
	...
	*new { arg context, doneCallback;
	...
	alloc {
		...
		folder = Platform.userHomeDir ++ "/dust/code/ambiance/audio/";
		...
		this.addCommand("stability", "f", { |msg|
			if(synth.notNil) { synth.set(\stability, msg[1]) };
		});
	...
	buildDef {
		SynthDef(\ambiance, {
			arg out = 0, blend = 0, stability = 0, speed = 1, volume = 1,
	...
	loadBuffers {
		Routine {
			...
			if(anyTone) {
				("ambiance: using test tones for missing files — add audio to "
					++ folder).postln;
			} {
				"ambiance: all six loops loaded".postln;
			};
		}.play;
	}
